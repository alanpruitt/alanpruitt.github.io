...
1. **Alignment Verification:** Verifies that every module assignment explicitly maps to a validated Course Learning Outcome (CLO).
2. **Deterministic Constraint Checking:** Ensures clinical calculation thresholds and SSoT rules (e.g., PAGA 2018 or ACSM 12th Ed.) are preserved without human tampering.
3. **WCAG 2.2 AA & Born-Accessible Audits:** Lints every line of HTML and Markdown to enforce high contrast, strict semantic heading structures (`<h2>` through `<h4>`), and descriptive media alt text before deployment.
